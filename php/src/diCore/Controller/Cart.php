...
class Cart extends \diBaseController
{
    ...
    {
        $this->cart = Model::autoCreate()->save();
    ...
    {
        if (!$this->cartItem) {
            $this->cartItem = $this->getCart()->getItem(
                $this->targetType,
                $this->targetId
            );
        }
    ...
    {
        $this->getCartItem()->setQuantity($this->quantity);
