...
class Comment extends \diBaseController
{
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            $target = new \diModel(
                $this->targetId,
                \diTypes::getTable($this->targetType)
            );
    ...
    protected function extendAddResult($result)
    {
        return extend($result, [
            ...
            'total_count' => $result['ok']
                ? $this->Comments->incTotalCount()
                : $this->Comments->getTotalCount(),
    ...
    public function refreshAction()
    {
        ...
        switch ($where) {
            case 'past':
                ...
                $comments = $this->Comments->getPastCommentsCollection(
                    $firstCommentId
                );
    ...
    protected function getTargetCommentOwner()
    {
        if (!$this->targetCommentOwner) {
            $this->targetCommentOwner = \diModel::create(
                \diTypes::user,
                $this->getTargetComment()->getUserId()
            );
        }
    ...
    protected function getCommenter()
    {
        if (!$this->commenter) {
            ...
            $this->commenter = \diModel::create(
                \diTypes::user,
                $this->getNewComment()->getUserId()
            );
        }
    ...
    protected function getTargetComment()
    {
        if (!$this->targetComment) {
            $this->targetComment = \diModel::create(
                \diTypes::comment,
                $this->getNewComment()->getParent()
            );
        }
