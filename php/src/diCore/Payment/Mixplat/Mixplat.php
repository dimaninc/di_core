...
class Mixplat
{
    ...
    private function handle($logTag)
    {
        ...
        $isSignCorrect =
            $sign &&
            $sign ==
                $this->calculateSign($_SERVER['REQUEST_URI'], $this->getPost());

        $this->log(
            $logTag,
            print_r(
                [
                    'is_sign_correct' => $isSignCorrect ? 'yes' : 'no',
                    'data' => $this->getPost(),
                ],
                true
            )
    ...
    private function query($method, $additionalParameters = [])
    {
        ...
        $this->log(
            'query',
            '--> ' . print_r(['url' => $url, 'parameters' => $parameters], true)
        );
    ...
    private function log($tag, $message)
    {
        if ($this->logDirectory !== null) {
            ...
            $h = fopen(
                $this->logDirectory . $tag . date('-Y-m-d') . '.txt',
                'a'
            );
            fwrite($h, $string);
            fclose($h);
