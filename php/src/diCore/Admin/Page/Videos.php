...
class Videos extends \diCore\Admin\BasePage
{
    ...
    {
        $this->getTpl()->define('`videos/form', ['after_form']);
    ...
    public function submitForm()
    {
        $this->getSubmit()
            ...
            ->storeFile($this->videoFields, function (
                ...
                FileSystemHelper::createTree(
                    \diPaths::fileSystem($this->getSubmit()->getModel()),
                    $folder,
                    Submit::DIR_CHMOD
                );
        ...
        if ($this->getSubmit()->getData('vendor') != Vendor::OWN) {
            $videoInfo = Vendor::extractInfoFromEmbed(
                stripslashes($this->getSubmit()->getData('embed'))
            );
            ...
            if ($videoInfo['vendor']) {
                $this->getSubmit()->setData('vendor', $videoInfo['vendor']);
            }
