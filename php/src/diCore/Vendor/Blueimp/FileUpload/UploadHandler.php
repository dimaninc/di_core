...
namespace diCore\Vendor\Blueimp\FileUpload;
...
class UploadHandler
{
    ...
    public function __construct(
        ...
        $this->options = [
            ...
            'redirect_allow_target' =>
                ...
                preg_quote(
                    parse_url(
                        $this->get_server_var('HTTP_REFERER'),
                        PHP_URL_SCHEME
                    ) .
                        '://' .
                        parse_url(
    ...
    protected function get_upload_path($file_name = null, $version = null)
    {
        ...
        if (empty($version)) {
            ...
            $version_dir = @$this->options['image_versions'][$version][
                'upload_dir'
            ];
            if ($version_dir) {
    ...
    protected function get_download_url(
        ...
        if (empty($version)) {
            ...
            $version_url = @$this->options['image_versions'][$version][
                'upload_url'
            ];
            if ($version_url) {
                return $version_url .
    ...
    protected function get_file_objects($iteration_method = 'get_file_object')
    {
        ...
        return array_values(
            array_filter(
                array_map([$this, $iteration_method], scandir($upload_dir))
            )
        );
    }
    ...
    protected function get_unique_filename(
        ...
        while (is_file($this->get_upload_path($name))) {
            if (
                $uploaded_bytes ===
                $this->get_file_size($this->get_upload_path($name))
            ) {
                break;
            }
            $name = $this->upcount_name($name);
    ...
    protected function gd_create_scaled_image($file_name, $version, $options)
    {
        ...
        list($file_path, $new_file_path) = $this->get_scaled_image_file_paths(
            $file_name,
            $version
        );
    ...
    protected function imagick_get_image_object($file_path, $no_cache = false)
    {
        if (empty($this->image_objects[$file_path]) || $no_cache) {
            ...
            if (!empty($this->options['imagick_resource_limits'])) {
                foreach (
                    $this->options['imagick_resource_limits']
                    as $type => $limit
                ) {
    ...
    protected function imagick_create_scaled_image(
        $file_name,
        $version,
        $options
    ) {
        ...
        if ($crop) {
            ...
            if ($img_width / $img_height >= $max_width / $max_height) {
                ...
                $x =
                    ($img_width / ($img_height / $max_height) - $max_width) / 2;
            } else {
                $new_height = 0; // Enables proportional scaling based on max_width
                $y =
                    ($img_height / ($img_width / $max_width) - $max_height) / 2;
            }
        }
    ...
    protected function imagemagick_create_scaled_image(
        $file_name,
        $version,
        $options
    ) {
    ...
    {
        if ($this->options['image_library'] === 2) {
            return $this->imagemagick_create_scaled_image(
                $file_name,
                $version,
                $options
            );
        ...
        if ($this->options['image_library'] && extension_loaded('imagick')) {
            return $this->imagick_create_scaled_image(
                $file_name,
                $version,
                $options
            );
    ...
    protected function download()
    {
        ...
        $file_path = $this->get_upload_path(
            $file_name,
            $this->get_version_param()
        );
        ...
        $this->header(
            'Last-Modified: ' .
                gmdate('D, d M Y H:i:s T', filemtime($file_path))
        );
    ...
    protected function send_content_type_header()
    {
        ...
        if (
            strpos($this->get_server_var('HTTP_ACCEPT'), 'application/json') !==
            false
        ) {
    ...
    protected function send_access_control_headers()
    {
        ...
        $this->header(
            'Access-Control-Allow-Credentials: ' .
                ($this->options['access_control_allow_credentials']
                    ? 'true'
                    : 'false')
        );
        $this->header(
    ...
    public function get($print_response = true)
    {
        ...
        if ($file_name) {
            $response = [
                $this->get_singular_param_name() => $this->get_file_object(
                    $file_name
                ),
            ];
        } else {
    ...
    public function post($print_response = true)
    {
        ...
        $content_disposition_header = $this->get_server_var(
