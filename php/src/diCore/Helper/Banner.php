...
class Banner
{
    ...
    public static function redirect()
    {
        ...
        $b_r = $bid
            ? self::getDb()->r(
                self::table,
                "WHERE id='$bid' and NOW() BETWEEN date1 and date2 and visible='1'"
            )
            : null;
    ...
    public static function printForPlace($place)
    {
        ...
        foreach ($banners as $banner) {
            ...
            self::getTpl()->parse(
                strtoupper($banner->getPlace()) . '_BANNER_ROWS',
                ".{$banner->getPlace()}_banner_row"
            );
        ...
        return self::getTpl()->parse_if_not_empty(
            strtoupper($place) . '_BANNERS_BLOCK',
            strtoupper($place) . '_BANNER_ROWS'
        );
    }
    ...
    public static function getAll($shuffle = false)
    {
        ...
        foreach (self::$placesAr as $place => $title) {
            $limits[$place] = (int) \diConfiguration::safeGet(
                "banners_max_count[$place]",
                0
            );
            $counts[$place] = 0;
        }
