...
class FastTemplate
{
    ...
    public $data = []; // holds parsed and assigned data
    public $cache_filename = '';
    public $cacheUsed = false;
    public $TEMPLATES = []; // holds unparsed templates (cache)
    public $FILELIST = []; // Holds the array of filehandles FILELIST[HANDLE] == "fileName"
    public $PARSEVARS = []; // Holds the array of Variable handles. PARSEVARS[HANDLE] == "value"
    public $LOADED = []; // We only want to load a template once - when it's used.
    // LOADED[FILEHANDLE] == 1 if loaded undefined if not loaded yet.
    public $HANDLE = []; // Holds the handle names assigned by a call to parse()
    ...
    public function __construct(
        ...
        $encoding = \diCore\Data\Http\Charset::title(
    ...
    private function get_cache_template_line(
        $filename,
        $basedir,
        $filename_prefix = ''
    ) {
    ...
    public function show_unknowns($line)
    {
        ...
        if (preg_match('/({[A-Z0-9_]+})/', $line, $unknown)) {
            ...
            if (!empty($UnkVar)) {
                @error_log(
                    "[FastTemplate] Warning: no value found for variable: $UnkVar ",
                    0
                );
            }
        }
    ...
    public function parse($returnVar, $fileTags = false)
    {
        ...
        if (gettype($fileTags) == 'array') {
            ...
            foreach ($fileTags as $val) {
                ...
                $this->data[$returnVar] = $this->parse_template(
                    $this->data[$val],
                    $this->PARSEVARS
                );
    ...
    public function clear_tpl($fileHandle = '')
    {
        ...
        if (empty($fileHandle)) {
            ...
            if (!is_array($fileHandle)) {
                if (
                    isset($this->data[$fileHandle]) ||
                    !empty($this->data[$fileHandle])
                ) {
