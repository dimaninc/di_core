...
/*
    ...
    // 2011/01/13
        * dynamic pics
        * ::prepare_checkboxed_field() added
...
class diAdminSubmit
{
	...
	public function gatherData()
	{
		...
		foreach ($this->_all_fields as $f => $v)
		{
			...
			switch ($v["type"])
			{
				...
				case "float":
					$this->processData($f, function($v) {
						return floatval(diStringHelper::fixFloatDot($v));
					});
					break;

				case "double":
					$this->processData($f, function($v) {
						return doubleval(diStringHelper::fixFloatDot($v));
					});
					break;
		...
		if ($this->id)
		{
			foreach ($this->_all_fields as $f => $v)
			{
				...
				else
				{
					...
					{
						$dbAr[$f] = diStringHelper::in($this->getData($f));
	...
	public function storeImage($field, $filesOptions = [])
	{
		...
		{
			$suffix = diAdminSubmit::getPreviewSuffix($opts["type"]);
...
function diasStoreImage(&$obj, $field, $options, $F)
{
	...
	foreach ($options as $opts)
	{
		...
		chmod($fn, diAdminSubmit::FILE_CHMOD);
...
{
	return unsharpmask($img, 80, 0.5, 0);
}

function dias_save_file($F, $field, $pics_folder, $fn, diAdminSubmit $obj)
{
