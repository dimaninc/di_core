...
class diAdminList
{
    ...
    public static $lngStrings = [
        'en' => [
            ...
            'notes_about_children' =>
                'Warning: if selected row has subrows, all actions will be applied to them as well',
        ],
        ...
        'ru' => [
            ...
            'notes_about_children' =>
                'Внимание, если у выделенной строки имеются подразделы, то все действия автоматически будут примененыи и для них',
        ],
    ];
    ...
    {
        $this->insertColumnsBefore($name, $newColumns)->removeColumn($name);
    ...
    {
        $this->insertColumnsBefore($name, [
            $newName => $this->columnsAr[$name],
        ])->removeColumn($name);
    ...
    {
        $this->columnsAr = ArrayHelper::addItemsToAssocArrayBeforeKey(
            $this->columnsAr,
            $name,
            $newColumns
        );
    ...
    {
        $this->columnsAr = ArrayHelper::addItemsToAssocArrayAfterKey(
            $this->columnsAr,
            $name,
            $newColumns
        );
    ...
    public function addRow($r, $options = [])
    {
        ...
        $this->setCurRec($r)->prepareReplaceAr();
        ...
        foreach ($this->columnsAr as $name => $properties) {
            ...
            if (
                isset($properties['headAttrs']) &&
                is_callable($properties['headAttrs'])
            ) {
                $properties['headAttrs'] = $properties['headAttrs']($r);
            }

            if (
                isset($properties['bodyAttrs']) &&
                is_callable($properties['bodyAttrs'])
            ) {
            ...
            if ($name == 'id') {
                ...
                switch ($name) {
                    ...
                    case 'manage':
                        ...
                        $html .= $this->T->$method(
                            $this->replaceValues($this->buildHref($p['href'])),
                            $p['icon']
                        );
                        break;
                ...
                $method =
                    empty($properties['noLink']) && empty($properties['noHref'])
                        ? 'textLinkCell'
                        : 'textCell';
    ...
    {
        return $this->T->openTable(
            $this->getOption('showHeader')
                ? diNiceTable::PRINT_HEADLINE
                : diNiceTable::NO_HEADLINE
        ) .
            $html .
            $this->T->closeTable();
    }
    ...
    protected function getControlPanelHtml()
    {
        ...
        return $this->getAdminPage()
            ->getTwig()
            ->getAssigned('list_control_panel') ?:
            $this->getAdminPage()
                ->getTwig()
                ->parse('admin/_index/list/control_panel', [
