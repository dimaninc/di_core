...
class diSearcher
{
    ...
    protected $rc_ar = [];
    protected $search_ids_ar = [];
    public $orig_search_ids_ar = [];
    ...
    protected function search()
    {
        ...
        foreach ($this->tables_ar as $table => $caption) {
            ...
            $search =
                $this->search_mode == 'db'
                    ? new diDBSearch($table)
                    : new diTextfileSearch($table);

            if ($search_r) {
                $search->search_id = $search_r->id;

                $search_result_r = $db->r(
                    'search_results',
                    "WHERE search_id='$search_r->id'",
                    'COUNT(*) AS cc'
                );
    ...
    protected function get_results_by_table($table)
    {
        ...
        $whereAr = ["i.search_id='{$this->search_ids_ar[$table]}'"];
        ...
        $query =
            ...
            ' ' .
            $props['order'] .
            " LIMIT $pn->start,$pn->per_page";

        $results = $db->rs(
            "$table t INNER JOIN search_results i ON t.id=i.id {$props['joins']}",
            $query,
            't.*,i.rel'
        );
