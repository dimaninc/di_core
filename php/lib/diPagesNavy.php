...
class diPagesNavy
{
    ...
    public function init(
        ...
        if ($table instanceof \diCollection) {
            ...
            $this->db = \diModel::createForTable($table)
                ::getConnection()
                ->getDb();
        }

        if (is_array($reverse)) {
            $this->init_on_last_page = isset($reverse['init_on_last_page'])
                ? $reverse['init_on_last_page']
                : false;
            $this->reverse = isset($reverse['reverse'])
                ? $reverse['reverse']
                : false;
        } else {
    ...
    public function get_page_of($id, $orderByField, $dir)
    {
        ...
        if ($col->getQuery()) {
            $col->setQuery(
                $col->getQuery() .
                    " AND {$this->getDb()->escapeField(
                        $orderByField
                    )} $sign {$this->getDb()->escapeValue($orderByValue)}"
            );
        } else {
            $col->filterBy($orderByField, $sign, $orderByValue);
        }
    ...
    public function extended_get_page_of($id, $orderby, $dir)
    {
        ...
        $rs2 = $this->getDb()->rs(
            $this->table,
            "$where ORDER BY $orderby $dir,date $dir",
            'id'
        );
    ...
    public function get_sibling_r($position, $id, $orderby, $dir, $circular = true)
    {
        ...
        $sign =
            (strtolower($dir) == 'asc' && $position == 'next') ||
            (strtolower($dir) == 'desc' && $position == 'prev')
                ? '>'
                : '<';
    ...
    {
        $href =
            ($this->reverse && $p == $this->total_pages) ||
            (!$this->reverse && $p == 1)
                ? $base_uri
    ...
    public function print_pages(
        ...
        if ($this->total_pages > 1) {
            ...
            if ($this->reverse) {
                $_pages['ar'] = array_reverse($_pages['ar']);
            }
            ...
            for ($i = 0; $i < count($_pages['ar']); $i++) {
                ...
                if ($p == $dots) {
                    ...
                    $tmp_s = str_replace(
                        ['{PAGE}', '{HREF}'],
                        [$p, $this->get_page_href($p, $base_uri)],
                        $this->tpl_ar[$tpl]
                    );
                }

                if ($separator && $i != count($_pages['ar']) - 1) {
                    $tmp_s .= $separator;
                }
        ...
        if ($this->reverse) {
            $prev_tpl =
                $prev_page <= $this->total_pages ? 'border-link' : 'inactive';
            $next_tpl = $next_page >= 1 ? 'border-link' : 'inactive';

            $prev_page_s = $this->str_ar['prev_symb']
                ? str_replace(
                    ['{PAGE}', '{HREF}'],
                    [
                ...
                : '';
            $next_page_s = $this->str_ar['next_symb']
                ? str_replace(
                    ['{PAGE}', '{HREF}'],
                    [
            ...
            $next_tpl =
                $next_page <= $this->total_pages ? 'border-link' : 'inactive';
        ...
        return $this->total_pages >= 1
            ? "$prev_page_s $separator $s $separator $next_page_s"
            : '';
    }
}
