...
/*
    ...
    // 2012/01/19
        * ct_ar array now has its own cache file

    // 2009/08/11
        * ::incut_albums(), ::incut_references() added

    // 2008/11/22
        * multilanguage vars support added

    // 2008/10/16
        * ::m() added

    // 2008/10/06
        * ::parse_content_page() added

    // 2008/10/06
        * ::set_relative_path() added

    // 2008/10/03
        * ::get_children_ids_ar() added
...
/** @deprecated */
define("DICMS_TPL_DIR", "tpl");
/** @deprecated */
define("DICMS_TPL_CACHE_PHP", "_cfg/cache/tpl_cache.php");

/** @deprecated */
define("DICMS_TABLES_CONTENT_CACHE_PHP", "_cfg/cache/table_content.php");
/** @deprecated */
define("DICMS_TABLES_CONTENT2_CACHE_PHP", "_cfg/cache/table_content2.php");

/** @deprecated */
define("DICMS_TABLES_CONTENT_CLEAN_TITLES_PHP", "_cfg/cache/table_content_ct_ar.php");

abstract class diCMS
{
	const TPL_DIR = "tpl";
	...
	public static function getCleanTitlesAr()
	{
		...
		{
			$z_ct_ar = [];
			include $_SERVER["DOCUMENT_ROOT"] . "/" . diCMS::TABLES_CONTENT_CLEAN_TITLES_PHP;
	...
	protected function printCommentsForPage()
	{
		...
		{
			$this->printCommentsBlock(diTypes::content, $this->getContentModel()->getId(), "PAGE_COMMENTS_BLOCK");
		}
	...
	{
		$this->getTpl()->assign([
			"SEARCH_Q" => trim(diStringHelper::out(diRequest::get("q", ""))),
		]);
	...
	protected function getCanonicalAddress($fullAddress = null)
	{
		...
		$fullAddress = diStringHelper::removeQueryStringParameter($fullAddress, [], [diPagesNavy::PAGE_PARAM, diComments::PAGE_PARAM]);
  ...
  public function load_content_rec()
  {
	...
	public function setMeta($text, $field = "title")
	{
		if (is_array($text))
		{
			$text = diArrayHelper::recursiveJoin($text, ' ');
		}
