...
class diForm
{
    ...
    function remove_local_params($params_ar = [])
    {
        ...
        foreach ($this->local_params as $i => $a) {
            if (in_array($a['field'], $params_ar)) {
                unset($this->local_params[$i]);
            }
        }

        foreach ($this->all_params as $i => $a) {
            if (in_array($a['field'], $params_ar)) {
                unset($this->all_params[$i]);
            }
        }
    }

    function remove_submit_params($params_ar = [])
    {
        if (!is_array($params_ar)) {
            $params_ar = [$params_ar];
        }
    ...
    function define_errors_of_submit_params($array_of_additional_keys = [])
    {
        foreach ($this->submit_params as $p) {
    ...
    function process_data($r = false)
    {
        ...
        foreach ($this->all_params as $p) {
            ...
            $this->data[$f] =
                isset($GLOBALS[$var][$f]) && !is_array($GLOBALS[$var][$f])
    ...
    function set_random_code_field(
        $field,
        $random_code_img_w = 0,
        $random_code_img_h = 0
    ) {
        ...
        $this->define_errors_of_submit_params([$field]);
    ...
    function check_random_code($error_msg)
    {
        ...
        $random_code = isset($_POST[$this->random_code_field])
            ? $_POST[$this->random_code_field]
            : '';
        $random_code_hash = isset($_POST["{$this->random_code_field}_hash"])
            ? $_POST["{$this->random_code_field}_hash"]
            : '';
    ...
    function check_random_code2($error_msg)
    {
        ...
        if (isset($_POST[$this->random_code_field])) {
            $id = isset($_POST[$this->random_code_field . '_log_id'])
                ? substr(
                    $_POST[$this->random_code_field . '_log_id'],
                    4,
                    strlen($_POST[$this->random_code_field . '_log_id']) - 8
                )
                : 0;
            ...
            if (
                $r &&
                static::get_captcha_uid($r->captcha_id) ==
                    $_POST[$this->random_code_field]
            ) {
                $ok = true;
            }
    ...
    function store()
    {
        ...
        if (
            $this->cur_rec &&
            !empty($this->cur_rec->id) &&
            substr($this->cur_rec->id, 0, 2) != '__' &&
            (int) $this->cur_rec->id
        ) {
    ...
    function assign_tpl_values(
        ...
        foreach ($this->all_params as $p) {
            ...
            if (empty($this->data[$f])) {
                $this->data[$f] = $cur_rec ? $cur_rec->$f : '';
            }
            //if ($this->data[$f] == 0 && in_array($t, explode(",", "float,double,int,integer,tinyint,smallint"))) $this->data[$f] = "";

            $tpl->assign(
                strtoupper($f) . $tpl_value_suffix,
                stripslashes($this->data[$f])
            );

            $tpl->assign([
                strtoupper($f) . '_CHECKED' =>
                    $t == 'checkbox' && $this->data[$f]
                        ? " checked=\"checked\""
                        : '',
            ]);
    ...
    function assign_tpl_random_code(&$tpl)
    {
        ...
        if (!empty($tpl->FILELIST['captcha_row'])) {
            ...
            $captcha_log_id = $db->insert('captchas_log', [
                'ip' => ip2bin(),
            ...
            foreach ($idx_ar as $idx) {
                ...
                $uid =
                    $captcha_id == $ar['id']
                        ? static::get_captcha_uid($captcha_id)
                        : get_unique_id();
                $uids_ar[] = "'$uid'";
    ...
    function assign_tpl_errors(
        &$tpl,
        $token = 'ERROR_TEXT',
        $tpl_token_suffix = '_ERROR_LINE',
        $tpl_name = 'error_line'
    ) {
    ...
    */
    ...
    function get_new_order_num($where = '', $where2 = '', $asc = true)
    {
        ...
        if ($order_r = $db->r($this->table, "{$where}{$where2}", 'order_num')) {
            ...
            $order_r = $db->r(
                $this->table,
                $where,
                "$max(order_num) AS num,COUNT(id) AS cc"
            );
    ...
    function get_clean_title($title, $field, $current_id = 0)
    {
        ...
        while (
            $db->count(
                $db->rs(
                    $this->table,
                    "WHERE $field='$title' and id!='$current_id'",
                    'id'
                )
            )
        ) {
    ...
    function parse_form_params($ar)
    {
        ...
        foreach ($ar as $p) {
            ...
            list($f, $t, $title) = array_merge(explode('|', $p), ['', '', '']);
            $x = strpos($f, '#');
            if ($x !== false) {
                list($old, $f) = explode('#', $f);
                if (isset($GLOBALS[$var][$old])) {
                    $GLOBALS[$var][$f] = $GLOBALS[$var][$old];
                }
            }
