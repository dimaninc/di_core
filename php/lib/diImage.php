...
class diImage
{
	...
	{
		$this->orig_fn = $fn;
		list($this->w, $this->h, $this->t) = is_file($fn) ? getimagesize($fn) : [0, 0, 0];

		if (
			$this->t >= 1 && $this->t <= 3 && $this->w * $this->h > self::MAX_GD_WIDTH * self::MAX_GD_HEIGHT &&
			class_exists('IMagick')
		   )
		{
			$im = new \Imagick($fn);
			$im->resizeImage(self::MAX_GD_WIDTH, self::MAX_GD_HEIGHT, \Imagick::FILTER_CATROM, 1, true);
			$im->writeImage();
			$im->clear();
			$im->destroy();
		...
		if ($this->t >= 1 && $this->t <= 3 && $this->w * $this->h <= self::MAX_GD_WIDTH * self::MAX_GD_HEIGHT)
		{
	...
	function read_info()
	{
		switch ($this->t)
		{
			case 1:
				...
				if ($this->info->m_version == "89a" && $this->info->m_colorFlag == 1)
				{
					$this->transparent = ImageColorAllocate($this->image,
	...
	function sharpen()
	{
		...
		unsharpmask($this->image, 80, 0.5, 0);
	}
	...
	function merge_wm_to(&$image, $wm_fn, $x = "left", $y = "top")
	{
		...
		{
			list($wm_w, $wm_h, $wm_t) = is_file($wm_fn) ? getimagesize($wm_fn) : array(0, 0, 0);
		}

		if (!$wm_w) return false;

		if ($wm_t == 3)
			imagealphablending($image, true);

		if ($x == "left") $x = 0;
		elseif ($x == "right") $x = imagesx($image) - $wm_w;
		elseif ($x == "center") $x = (imagesx($image) - $wm_w) >> 1;
		else $x = $x >= 0 ? $x : imagesx($image) - $wm_w + (int)$x;

		if ($y == "top") $y = 0;
		elseif ($y == "bottom") $y = imagesy($image) - $wm_h;
		elseif ($y == "center") $y = (imagesy($image) - $wm_h) >> 1;
		else $y = $y >= 0 ? $y : imagesy($image) - $wm_h + (int)$y;
	...
	function get_flipped_horizontal()
	{
		...
		for ($x = 0; $x < $this->w; $x++)
		{
			imagecopy($dst_img, $this->image, $x, 0, $this->w - $x - 1, 0, 1, $this->h);
		}
	...
	function get_flipped_vertical()
	{
		...
		for ($y = 0; $y < $this->h; $y++)
		{
			imagecopy($dst_img, $this->image, 0, $y, 0, $this->h - $y - 1, $this->w, 1);
		}
	...
	function calculate_dst_dimentsions($mode, $w, $h)
	{
		...
		return array($src_w, $src_h, $src_x, $src_y, $dst_w, $dst_h, $dst_x, $dst_y);
	}

	public static function createFunction($img_type)
	{
		$func_suffix = $img_type >= 1 && $img_type <= 3 ? self::$typeNames[$img_type] : "";
