...
class diComments
{
    ...
    protected function initPagesNavy()
    {
        $this->PagesNavy = new \diPagesNavy(
            ...
            [
                'initial' =>
                    $this->getMode() == self::MODE_INITIAL
                        ? $this->getRecentCommentsCount()
                        : $this->getCommentsCountPerLoad(),
                'load' => $this->getCommentsCountPerLoad(), // this is not used yet
            ],
            'WHERE ' . $this->getBaseQuery(),
            false,
            static::PAGE_PARAM
        );
    ...
    public function printForm()
    {
        ...
        if (
            $tplName &&
            $this->getTpl() &&
            $this->getTpl()->defined($tplName) &&
            $this->getTpl()->exists($tplName)
        ) {
            $this->getTpl()
                ->assign(
                    [
                        ...
                        'MODERATED_BEFORE_SHOW' => static::moderatedBeforeShow()
                            ? 'true'
                            : 'false',
                    ],
                    'COMMENT_'
    ...
    {
        $page =
            $this->usePagesNavy && $this->getPagesNavy()
                ? $this->getPagesNavy()->getPage()
                : null;
    ...
    protected function prepareToRenderBlock()
    {
        ...
        $this->getTpl()->assign([
            ...
            'COMMENT_AUTH_CLASS_NAME' => $this->authorized()
                ? 'auth-ok'
                : 'auth-needed',
        ]);
    ...
    protected function getUserModelForComment(CommentModel $comment)
    {
        if ($this->admins && $this->users) {
            $user =
                $comment->getUserType() == self::utAdmin
    ...
    public function getRowHtml(CommentModel $comment)
    {
        ...
        return $this->getTpl()->parse('comment_row');
    }
    ...
    {
        return $this->getCommentsCollection(
            array_merge(
                $this->queryAr,
                $this->getAdditionalQueryAr(),
                $this->getInitialQueryAr()
            )
        );
    }
    ...
    public function getPastCommentsCollection($firstCommentId, $size = null)
    {
        return $this->getCommentsCollection(
            array_merge(
                $this->queryAr,
                $this->getAdditionalQueryAr(),
                $this->getPastQueryAr($firstCommentId)
            ),
            $size
        );
    }
    ...
    protected function getCommentsCollection($queryAr, $size = null)
    {
        ...
        $this->comments = \diCollection::create(
            \diTypes::comment,
        ...
        $this->users = \diCollection::create(\diTypes::user)->filterById(
            array_unique($userIds)
        );
        $this->admins = \diCollection::create(\diTypes::admin)->filterById(
            array_unique($adminIds)
        );
    ...
    {
        $queryAr = array_merge($this->queryAr, ["visible = '1'"]);
    ...
    public function cacheNeededByTargetType()
    {
        ...
        if (
            !static::$cachedTargetTypes ||
            in_array($this->targetType, static::$cachedTargetTypes)
        ) {
            $res = true;
        }
