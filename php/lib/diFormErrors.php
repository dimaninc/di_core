...
class diFormErrors
{
    ...
    {
        if (
            isset($this->errors_array[$key]) &&
            count($this->errors_array[$key])
        ) {
    ...
    function define($key)
    {
        if (is_array($key)) {
            foreach ($key as $k) {
                if (
                    !isset($this->errors_array[$k]) ||
                    !is_array($this->errors_array[$k])
                ) {
                    $this->errors_array[$k] = [];
                }
            }
            ...
            if (
                !isset($this->errors_array[$key]) ||
                !is_array($this->errors_array[$key])
            ) {
                $this->errors_array[$key] = [];
            }
        }
    }
}
