...
class diSwiffy
{
    ...
    {
        return strtolower(
            StringHelper::fileExtension(self::normalizeFilename($filename))
        ) == 'html';
    }
    ...
    public static function getDimensions($filename)
    {
        if (
            preg_match(
                self::dimensionsRegEx,
                file_get_contents(self::normalizeFilename($filename)),
                $regs
            )
        ) {
    ...
    public static function getHtml($filename, $w = null, $h = null, $options = [])
    {
        ...
        return sprintf(
            '<iframe %s></iframe>',
            ArrayHelper::toAttributesString($attributes, false)
        );
    }
}
