...
class diEmail
{
    ...
    public static function parseNameAndEmail(
        $nameAndEmail,
        $defaultEmail = '',
        $defaultName = ''
    ) {
	...
	*/
    public static function fastSend(
        $from,
        $to,
        $subject,
        $message,
        ...
        $class = in_array(get_called_class(), ['\diEmail', 'diEmail'])
        ...
        // encoding
        $from = $fromName
            ? "=?{$headerEnc}?B?" . base64_encode($fromName) . "?= <$fromEmail>"
            : $fromEmail;
        $to = $toName
            ? "=?{$headerEnc}?B?" . base64_encode($toName) . "?= <$toEmail>"
            : $toEmail;
                ...
                foreach ($attachments as $attachment) {
                    if (empty($attachment['data']) && !empty($attachment['path'])) {
                        ...
                        if (empty($attachment['content_type'])) {
                            $attachment[
                                'content_type'
                            ] = \diCore\Helper\StringHelper::mimeTypeByFilename(
                                $attachment['filename']
                            );
                        }
                    }

                    $attachment['data'] = chunk_split(
