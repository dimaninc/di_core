...
class diFTP
{
    ...
    public function connect($host, $login, $password, $port = 21)
    {
        ...
        if (!ftp_login($this->ftp, $this->login, $this->password)) {
            $this->log(
                'fatal',
                "Unable to login to host $this->host:$this->port as $this->login"
            );
        }
    ...
    public function make_dir($dir, $silent = false)
    {
        if (
            !@ftp_mkdir(
                $this->ftp,
                add_ending_slash($this->dir) . $this->fixEncoding($dir)
            )
        ) {
            if (!$silent) {
    ...
    public function put(
        ...
        if (
            ...
            throw new \Exception(
                'FTP: Local and remote filenames count should match'
            );
        }
        ...
        foreach ($fn_ar as $idx => $fn) {
            ...
            if (!@ftp_put($this->ftp, $remote_fn, $fn, FTP_BINARY)) {
                $this->log(
                    'warning',
                    "Unable to put file $fn to $this->dir/$remote_fn"
                );
    ...
    public function simple_put($local_fn, $remote_fn)
    {
        ...
        if (!@ftp_put($this->ftp, $remote_fn, $local_fn, FTP_BINARY)) {
            $this->log(
                'warning',
                "Unable to put file $local_fn to $this->dir/$remote_fn"
            );
