...
class diLib
{
    ...
    public static $libSubFolders = [
        self::pathProjectLib => ['OAuth2', 'traits'],

        self::pathCoreLib => ['OAuth2', 'traits'],
    ];
    ...
    /** @deprecated */
    public static $classesAr = [];

    /** @deprecated */
    public static $classPropertiesAr = [];
    ...
    public static function getWorkerBasePath(
        $controller = null,
        $action = null,
        $paramsAr = null,
        $options = []
    ...
    public static function parentNamespace($namespace)
    {
        ...
        return $x === false ? '' : substr($namespace, 0, $x);
    ...
    public static function childNamespace($namespace)
    {
        ...
        return $x === false ? '' : substr($namespace, $x + 1);
    ...
    public static function getChildClass(
        ...
        foreach (self::getAllNamespaces() as $ns) {
            $class = self::getClassNameInNamespace(
                $basicName,
                $ns . $subNamespace,
                self::SIMPLE_CLASS
            );
    ...
    public static function getWorkerPath(
        $controller = null,
        $action = null,
        $paramsAr = null,
        $options = []
        ...
        return $subFolder .
            self::workerPrefix .
            self::getWorkerBasePath($controller, $action, $paramsAr, $options);
    }

    public static function getAdminWorkerPath(
        $controller = null,
        $action = null,
        $paramsAr = null,
        $options = []
    ...
    public static function normalizeClassName($className)
    {
        $x = array_search(
            ...
            array_map('strtolower', self::$classesAr)
        );

        return $x !== false ? self::$classesAr[$x] : null;
    }

    public static function getClassPathSubfolder($className)
    {
    ...
    public static function getLocation()
    {
        ...
        if (
            ...
            if (substr($__file__, 0, strlen($root)) === $root) {
                ...
                self::$subFolder = trim(
                    substr($__file__, 0, strpos($__file__, $marker)),
                    $sep
                );
            }
        }
    ...
    public static function getSubFolder($openingSlash = false)
    {
        ...
        $slash =
            $openingSlash === 'force' || (self::$subFolder && $openingSlash)
                ? '/'
                : '';
    ...
    public static function getClassFilename($className, $subFolder = '')
    {
        ...
        if (!$path) {
            ...
            foreach (self::$libPathsAr as $folderId => $folderPath) {
                ...
                if (!empty(self::$libSubFolders[$folderId])) {
                    $libSubFolders = array_merge(
                        $libSubFolders,
                        self::$libSubFolders[$folderId]
                    );
                }
    ...
    public static function exists($className, $subFolder = '')
    {
     ...
     * @return bool
     */
    public static function inc($className, $subFolder = '')
    {
    ...
    /** @deprecated */
    public static function incInterface($interfaceName, $subFolder = '')
    {
