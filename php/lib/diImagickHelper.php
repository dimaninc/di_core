...
class diImagickHelper
{
    ...
    public static function printWrappedText(\Imagick $im, $text, $options = [])
    {
        ...
        if ($options['debug']) {
            Logger::getInstance()->log(
                "{$options['font']} lineHeight calculated = $lineHeight, gotten from settings = " .
                    $options['fontLineHeight']
            );
            Logger::getInstance()->variable('$info', $info);
        }

        if (
            $options['fontLineHeight'] &&
            (!$lineHeight || $lineHeight > $options['fontLineHeight'])
        ) {
            $lineHeight = $options['fontLineHeight'];
        }

        $lineHeight =
            $lineHeight ?: $options['size'] * $options['fontLineHeight'];

        $options['debug'] &&
            Logger::getInstance()->log("final lineHeight = $lineHeight");
        ...
        list($deltaX, $deltaY) = $options['tuneCoordinatesCallback']
            ? $options['tuneCoordinatesCallback'](
                $options,
                $lineHeight * count($lines)
            )
            : [0, 0];
        ...
        for ($i = 0; $i < count($lines); $i++) {
            ...
            $options['debug'] &&
                Logger::getInstance()->log("[$x, $y]: {$lines[$i]}");
     ...
     */
    public static function getWordWrapInfo(
        \Imagick $image,
        \ImagickDraw $draw,
        $text,
        $maxWidth = null
        ...
        while ($i < count($words)) {
			...
			*/
            ...
            $metrics = $image->queryFontMetrics(
                $draw,
                $currentLine .
                    ($i < count($words) - 1 ? ' ' . $words[$i + 1] : '')
            );

            while (
                $metrics['textWidth'] <= $maxWidth &&
                $i < count($words) - 1
            ) {
                $currentLine .= ' ' . $words[++$i];
