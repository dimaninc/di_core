...
class diOAuth2Vk extends diOAuth2
{
	...
	protected function downloadData()
	{
		...
		$tokenInfo = json_decode(static::makeHttpRequest(static::authUrlBase, $this->getAuthUrlParams()), true);
		...
		if (count($tokenInfo))
		{
			if (isset($tokenInfo['access_token']))
			{
				$params = [
					...
					'fields' => 'uid,email,first_name,last_name,nickname,screen_name,sex,bdate,city,country,timezone,photo,photo_medium,photo_big,has_mobile,rate,contacts,education,online,counters',
				];
				...
				if ($data)
				{
					if (!empty($data['error']))
					{
						$this->setProfileError('Error retrieving data: ' . $data['error']['error_msg'] .
	...
	function simple_retrieve_profile()
	{
		...
		if (isset($_GET["code"]))
		{
			...
			$user_profile = json_decode(join(file(
		...
		{
			die(header("Location: https://oauth.vk.com/authorize?client_id={$this->options->app_id}&scope=".
