        ...
        if (!empty($tokenInfo->access_token)) {
            ...
            if ($token) {
                $params = [
                    'access_token' => $token,
                    'fields' =>
                        'first_name,last_name,middle_name,name,link,gender,timezone,locale,verified,picture,age_range,birthday,cover,email,id,hometown,languages,name_format,updated_time,website',
                ];

                $this->setProfileRawData(
                    json_decode(
                        static::makeHttpRequest(
                            static::profileUrlBase,
                            $params
                        ),
                        true
                    )
                ...
                $errorInfo = json_decode(
                    current(array_keys((array) $tokenInfo))
                );
