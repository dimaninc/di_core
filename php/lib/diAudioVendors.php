...
class diAudioVendors
{
    ...
    public static $patterns = [
        self::MixCloud =>
            '/mixcloud\.com\/((?!widget)[^\/]+)\/((?!iframe)[^\/]+)\/|mixcloud\.com%2F([^%]+)%2F([^%]+)%2F/',
    ...
    public static function getEmbedLink($vendor, $audioUid)
    {
        switch ($vendor) {
            case self::MixCloud:
                return 'https://www.mixcloud.com/widget/iframe/?feed=' .
                    urlencode(self::getLink($vendor, $audioUid)) .
    ...
    public static function getEmbedCode($vendor, $audioUid)
    {
        switch ($vendor) {
            case self::MixCloud:
                return sprintf(
                    "<iframe width=\"100%%\" height=\"120\" src=\"%s\" frameborder=\"0\"></iframe>",
                    self::getEmbedLink($vendor, $audioUid)
                );
        }
