var diAdminListControlPanel = function (_opts) {
    ...
    function setupToggleClicks() {
        e.$rowToggles.parent('td').click(function (e) {
            if (!$(e.target).is(':checkbox')) {
                $(this)
                    .find('input[type="checkbox"][data-purpose="toggle"]')
                    .click();
            }
        });
    }
    ...
    function setupButtons() {
        var $btn, action, title, ids, parent, confirmation;
        ...
        e.$buttonsContainer.find('button').on('click', function () {
            ...
            if (
                !confirm(confirmation + ' (' + ids.length + ' шт)?') ||
                !confirm('Вы уверены?')
            ) {
                return false;
            }
            ...
            if (in_array(action, ['copy', 'move'])) {
                ...
                $('.dinicetable_div').prepend(
                    '<div class="outer-anchor">{0}</div>'.format(anchorHtml)
                );
    ...
    function call(action, params, callback) {
        ...
        $.post(
            opts.NiceTable.getWorkerBase() + 'batch_' + action + '/' + table + '/',
            params,
            function (res) {
                if (!res.ok) {
                    var message =
                        'Error while requesting action "' +
                        action +
                        '" for #' +
                        params.ids;
    ...
    function checkButtonsVisibility() {
        ...
        e.$buttonsContainer.toggleClass('visible', selected());
