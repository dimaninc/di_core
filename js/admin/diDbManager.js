function diDbManager(_opts) {
    ...
    function bindEvents() {
        ...
        $(document.body)
            .on(
                'click',
                '[data-type="database"] button[data-action="restore"]',
                function () {
                    self.restoreDump(
                        $(this).closest('[data-filename]').data('filename'),
                        $(this).closest('[data-folder-id]').data('folder-id')
                    );
                }
            )
            .on(
                'click',
                '[data-type="database"] button[data-action="download"]',
                function () {
                    self.downloadDump(
                        $(this).closest('[data-filename]').data('filename'),
                        $(this).closest('[data-folder-id]').data('folder-id')
            ...
            .on(
                'click',
                '[data-type="database"] button[data-action="delete"]',
                function () {
                    self.deleteDump(
                        $(this).closest('[data-filename]').data('filename')
                    );
            ...
            .on(
                'click',
                '[data-type="database"] button[data-action="view"]',
                function () {
                    self.viewDump(
                        $(this).closest('[data-filename]').data('filename'),
                        $(this).closest('[data-folder-id]').data('folder-id')
        ...
        $.get(urlBase, options, function (res) {
            ...
            switch (action) {
                ...
                case 'delete':
                    ...
                    $(
                        '.db-dump-rows[data-type="database"] tbody tr[data-filename="' +
                            res.file +
                            '"]'
                    ).fadeOut();
    ...
    function addDumpRow(res) {
        var sizeStr = res.size ? ', ' + size_in_bytes(res.size) : '';

        $(
            '.db-dump-rows[data-type="database"] tbody[data-folder-id="1"] tr:first-child'
        ).after(
            '<tr data-format="' +
                res.format +
                '" data-filename="' +
    ...
    this.restoreDump = function (file, folderId) {
        if (
            !confirm(
                'Are your sure you want to restore this database dump?\nWarning! Current database might be deleted or damaged!'
            )
        ) {
            return false;
        }
    ...
    this.deleteDump = function (file) {
        if (
            !confirm(
                'Are you sure you want to delete the database dump ' + file + '?'
            )
        ) {
            return false;
        }
    ...
    this.refreshTableSizes = function () {
        ...
        $options.each(function () {
            if (this.selected) {
                var ar = (this.innerHTML || this.text).match(
                    /^[a-z0-9_]+,\s([^\s]+)\s\([^\s]+\s([^\s]+)\)/i
                );
