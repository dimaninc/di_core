...
function _add_event(elem, evname, func) {
  ...
  } else elem.addEventListener(evname, func, false); //true
}

function delEvent(elem, evname, func) {
  if (elem.attachEvent) {
    eval('elem.on' + evname + '=null;');
  } else elem.removeEventListener(evname, func, false); //true
}
...
function diCalendar(cfg) {
  ...
  this.get_prev_link_html = function () {
    ...
    if (this.cfg.prev_year_link_html)
      html +=
        '<p id="' +
        this.id +
        '_prev_year" class="prev-' +
        class_prefix +
        'active" onclick="' +
        this.instance_name +
        '.go_to_prev_year();">' +
        this.cfg.prev_year_link_html +
        '</p>';
  ...
  this.get_next_link_html = function () {
    ...
    if (this.cfg.next_link_html)
      html +=
        '<p id="' +
        this.id +
        '_next" class="next-active" onclick="' +
        this.instance_name +
        '.go_to_next_month();">' +
        this.cfg.next_link_html +
        '</p>';
    ...
    if (this.cfg.next_year_link_html)
      html +=
        '<p id="' +
        this.id +
        '_next_year" class="next-active" onclick="' +
        this.instance_name +
        '.go_to_next_year();">' +
        this.cfg.next_year_link_html +
        '</p>';
  ...
  this.get_close_link_html = function () {
    return this.cfg.close_link_html
      ? '<p id="' +
          this.id +
          '_close" class="dic-close" onclick="' +
          this.instance_name +
          '.hide();">' +
          this.cfg.close_link_html +
          '</p>'
      : '';
  };

  this.get_clear_link_html = function () {
    return this.cfg.clear_link_html
      ? '<p id="' +
          this.id +
          '_clear" class="dic-clear" onclick="' +
          this.instance_name +
          '.clear();">' +
          this.cfg.clear_link_html +
          '</p>'
      : '';
  };
  ...
  this.get_prompt_for_idx_div = function () {
    return (
      '<div id="' +
      this.id +
      '_prompt_for_idx" class="dicalendar-prompt-for-idx"></div>'
    );
  };
  ...
  this.prompt_for_idx_and_set_date = function (date) {
    ...
    if (e) {
      var date_ar =
        typeof date == 'string' && date ? date.split(/[\/\.:\x20]+/) : [];
      ...
      e.innerHTML =
        ...
        date +
        '\', 1);">' +
        this.cfg.date1_select_str.replace('%d', d).replace('%m', m) +
        '</div>' +
        '<div onclick="' +
        this.instance_name +
        ".set_date('" +
        date +
        '\', 2);">' +
        this.cfg.date2_select_str.replace('%d', d).replace('%m', m) +
        '</div>';
  ...
  this.get_str_date_from_input = function (e_obj) {
    ...
    if (e_obj.e) {
      ...
      if (e_obj.th && e_obj.tm) s += ' ' + e_obj.th.value + ':' + e_obj.tm.value;
    }

    return s;
  };

  this.setVal = function (inp, val) {
    inp.value = val;
  ...
  this.set_str_date_to_input = function (idx, date) {
    ...
    if (this.e_ar[idx]) {
      if (this.e_ar[idx].e) {
        ...
        if (typeof this.e_ar[idx].e.value != 'undefined') {
          this.setVal(this.e_ar[idx].e, s);
        } else if (typeof this.e_ar[idx].e.innerHTML != 'undefined') {
          this.e_ar[idx].e.innerHTML = s;
        }
      } else if (this.e_ar[idx].dd) {
        if (typeof dt == 'object') {
          this.setVal(this.e_ar[idx].dd, lead0(dt.getDate()));
          this.setVal(this.e_ar[idx].dm, lead0(dt.getMonth() + 1));
          this.setVal(this.e_ar[idx].dy, lead0(dt.getFullYear()));

          if (this.e_ar[idx].th && this.e_ar[idx].tm) {
            this.setVal(this.e_ar[idx].th, lead0(dt.getHours()));
            this.setVal(this.e_ar[idx].tm, lead0(dt.getMinutes()));
          }
        } else {
          this.setVal(this.e_ar[idx].dd, '');
          this.setVal(this.e_ar[idx].dm, '');
          this.setVal(this.e_ar[idx].dy, '');

          if (this.e_ar[idx].th && this.e_ar[idx].tm) {
            this.setVal(this.e_ar[idx].th, '');
            this.setVal(this.e_ar[idx].tm, '');
          }
        }
      }
    }
  ...
  this.get_e_ar = function (date) {
    ...
    if (typeof date == 'string') {
      ...
      if (ar.th)
        eval(
          "_add_event(ar.th, 'keyup', function() {" +
            this.instance_name +
            '.init();});'
        );
      if (ar.tm)
        eval(
          "_add_event(ar.tm, 'keyup', function() {" +
            this.instance_name +
            '.init();});'
        );
    ...
    if (ar && ar.e)
      eval(
        "_add_event(ar.e, 'keyup', function() {" + this.instance_name + '.init();});'
      );
  ...
  this.go_to_prev_month = function () {
    ...
    var a = {
      m: this.showing_dt.getMonth() + 1,
      y: this.showing_dt.getFullYear()
    };
  ...
  this.go_to_prev_year = function () {
    ...
    this.showing_dt = new Date(
      this.showing_dt.getFullYear() - 1,
      this.showing_dt.getMonth(),
      1,
      12,
      0,
      0,
  ...
  this.update_prev_next_buttons = function () {
    ...
    if (e1 && !this.cfg.able_to_go_to_past)
      e1.className =
        this.showing_dt <= this.today_dt ? 'prev-inactive' : 'prev-active';
  };
  ...
  this.set_date = function (date, idx) {
    ...
    if (this.range_select) {
      ...
      */
      ...
      if (e) {
        if (trim(e.innerHTML)) e.innerHTML += ', ';
        else e.innerHTML = '';
    ...
    if (!this.range_select && this.cfg.mode != 'multi') this.hide();
  ...
  this.str_to_obj = function (str) {
    ...
    if (date_ar[1] * 1 == 0 || isNaN(date_ar[1] * 1)) {
      date_ar[1] = ar_indexOf(
        date_ar[1],
        dicalendar_lng_ar[this.cfg.language].month_titles
      );
      if (date_ar[1] == -1)
        date_ar[1] = ar_indexOf(
          date_ar[1],
          dicalendar_lng_ar[this.cfg.language].nominative_month_titles
        );
    }
  ...
  this.get_month_html = function (m, y) {
    ...
    var days_to_show = Math.round(
      (m_finish.getTime() - m_start.getTime()) / 86400000
    );
    ...
    for (var i = 1; i <= weeks_to_show; i++) {
      ...
      for (var j = 1; j <= 7; j++) {
        ...
        if (
          (i > 1 && i < weeks_to_show) ||
          (i == 1 && j >= wd_of_m_start) ||
          (i == weeks_to_show && j <= wd_of_m_finish)
        ) {
          d++;
          yday++;
        ...
        if (j == 6 || j == 7) {
          if (big_yday < big_today_yday) {
            if (is_selected) class_attr = ' class="selected_past_weekend"';
            else class_attr = ' class="past_weekend"';

            in_past = true;
          } else if (is_selected) class_attr = ' class="selected_weekend"';
          else class_attr = ' class="weekend"';
        } else {
          if (big_yday < big_today_yday) {
            if (is_selected) class_attr = ' class="selected_past"';
            else class_attr = ' class="past"';
            in_past = true;
          } else if (is_selected) {
        ...
        day_id_attr += is_empty
          ? ''
          : ' onclick="' +
            this.get_day_onclick(lead0(d) + '.' + lead0(m) + '.' + y) +
            '"';
  ...
  this.get_date_str = function (date, output_type) {
    ...
    if (output_type == 'input')
      return (
        d + ' ' + dicalendar_lng_ar[this.cfg.language].month_titles[m] + ' ' + y
      );
    else if (output_type == 'uri') return y + '-' + lead0(m) + '-' + lead0(d);
    else if (output_type == 'calendar') return lead0(d) + '.' + lead0(m) + '.' + y;
    else return 'unknown output_type=' + output_type;
  };
  ...
  this.show = function () {
    ...
    if (typeof dip != 'undefined' && !this.cfg.no_gray) dip.showOverlay();
  ...
  this.hide = function () {
    ...
    if (typeof dip != 'undefined' && !this.cfg.no_gray) dip.hideOverlay();
    this.state = false;
  ...
  this.add_events_to_inputs = function () {
    ...
    for (var idx = 1; idx <= 2; idx++)
      if (
        this.cfg.add_events_to_date[idx] &&
        this.cfg.add_events_to_date[idx].length
      ) {
        var events_ar2 = in_array('all', this.cfg.add_events_to_date[idx])
          ? events_ar
          : this.cfg.add_events_to_date[idx];
        ...
        for (var i in events_ar2) {
          if (this.e_ar[idx].e) {
            eval(
              '_add_event(this.e_ar[idx].e, events_ar[i], function(){' +
                this.instance_name +
                '.' +
                method +
                '();});'
            ...
            eval(
              '_add_event(this.e_ar[idx].dd, events_ar[i], function(){' +
                this.instance_name +
                '.' +
                method +
                '();});'
            ...
            eval(
              '_add_event(this.e_ar[idx].dm, events_ar[i], function(){' +
                this.instance_name +
                '.' +
                method +
                '();});'
            ...
            eval(
              '_add_event(this.e_ar[idx].dy, events_ar[i], function(){' +
                this.instance_name +
                '.' +
                method +
                '();});'
            ...
            if (this.e_ar[idx].th && this.e_ar[idx].tm) {
              eval(
                '_add_event(this.e_ar[idx].th, events_ar[i], function(){' +
                  this.instance_name +
                  '.' +
                  method +
                  '();});'
              ...
              eval(
                '_add_event(this.e_ar[idx].tm, events_ar[i], function(){' +
                  this.instance_name +
                  '.' +
                  method +
                  '();});'
  ...
  this.e_ar =
    typeof cfg.date2 != 'undefined' && cfg.date2
  ...
  if (typeof cfg.stick_to == 'string') {
    ...
    if (!this.stick_to_e) this.stick_to_e = _ge(cfg.stick_to + '[dd]');

    if (!this.stick_to_e) this.stick_to_e = _ge(cfg.stick_to + '_d');
  } else if (typeof cfg.stick_to == 'object') this.stick_to_e = cfg.stick_to;
  else this.stick_to_e = this.e_ar[1].e ? this.e_ar[1].e : this.e_ar[1].dd;

  if (!this.stick_to_e) console.log('error! "stick to" element not found');
