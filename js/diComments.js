var diComments = function (_opts) {
    ...
    opts = $.extend(
      {
        ...
        showAfterSubmitMessage: function (o) {
          o.e.$form
            .find('[data-purpose="after-submit-message"]')
  ...
  function initElements() {
    if (opts.$container) {
      self.e.$container =
        typeof opts.$container == 'object' ? opts.$container : $(opts.$container);
    }

    if (opts.$defaultFormContainer) {
      self.e.$defaultFormContainer =
        typeof opts.$defaultFormContainer == 'object'
    ...
    self.e.$form = $(
      'form[name="comment_form"],[data-purpose="comment_form"]',
      self.e.$block
    );
  ...
  function initEvents() {
    ...
    if (opts.submitOnCtrlEnter) {
      self.e.$content
        .off('keypress.diComments')
        .on('keypress.diComments', function (event) {
    ...
    switch (data.action) {
      ...
      case 'edit':
        ...
        if (data.ok) {
          if (data.action == 'edit') {
            self
              .getCommentRow(self.e.$form.attr('data-id'))
              .find('.content')
              .text(self.val('content'));
  ...
  function initActions() {
    $('.actions [data-action]', self.e.$block)
      .off('click.diComments')
      .on('click.diComments', function () {
        var $this = $(this),
  ...
  this.refresh = function (_opts) {
    ...
    $.post(
      ...
      function (data) {
        ...
        if (
          self.e.$rows_container.find('.comment-row').length ==
          self.e.$block.data('total-count')
        ) {
          self.hideLoadPreviousBlock();
        }
      }
  ...
  function placeCommentRow(data, _opts) {
    ...
    if (data.parent > 0) {
      ...
      var method =
        (!data.dir || data.dir.toLowerCase() == 'asc') && !_opts.forcePrepend
          ? 'append'
          : 'prepend';
    ...
    if (
      _opts.scrollTo &&
      opts.scrollToNewCommentTimeout !== null &&
      $e.position().top
    ) {
      setTimeout(function () {
        $('html, body').animate(
