...
**Rows of a table-backed `'dynamic'` field are logged into the PARENT's record, not a record of their own.** `diDynamicRows::submit()` saves them with a bare `save()` and removes them with a raw `DELETE`, so nothing else sees them: it reads the rows by the field's own `subquery` before and after, hands both to `BasePage::addNestedEditLogSnapshot()`, and `buildEditLogRecord()` folds them in through `Model::addNestedRowsData()` – `field[id].column` for an edited column, `field[id]` with the row as JSON for an added or removed one (enough to restore it). Why the parent: the form's log tab reads only the parent's `target_table`/`target_id`, so a record per child row would exist and be seen by nobody. The gate is `BasePage::useEditLogForNestedEntities()`, which follows `useEditLog()` unless a page overrides it; a failure goes to `onNestedEditLogFailure()` and never costs the save. Not covered: lite `int[]`/`string[]` fields (no rows of their own) and `dynamic_pics`/`dynamic_files`. Covered by `php/tests/Admin/NestedEditLogTest.php` (the real `submit()` over a throwaway table) and `php/tests/Entity/AdminTableEditLog/NestedRowsLogTest.php`.
