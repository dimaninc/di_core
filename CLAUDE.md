...
**Rows of a table-backed `'dynamic'` field are logged into the PARENT's record, not a record of their own.** `diDynamicRows::submit()` saves them with a bare `save()` and removes them with a raw `DELETE`, so nothing else sees them: it reads the rows by the field's own `subquery` before and after, hands both to `BasePage::addNestedEditLogSnapshot()`, and `buildEditLogRecord()` folds them in through `Model::addNestedRowsData()` – `field[id].column` for an edited column, `field[id]` with the row as JSON for an added or removed one (enough to restore the row, but not its pic/file columns: `submit()` deletes those files from disk). Why the parent: the form's log tab reads only the parent's `target_table`/`target_id`, so a record per child row would exist and be seen by nobody. Expect `order_num` shifts in the record: after a deletion `submit()` renumbers the remaining rows, and each shifted row is a real stored change. The gate is `BasePage::useEditLogForNestedEntities()`, which follows `useEditLog()`; override it to switch nested logging off – switching it on without `useEditLog()` writes records the form's tab won't show. A failed snapshot read throws instead of reading as "no rows" (which would log every row as removed), and every failure goes to `onNestedEditLogFailure()` without costing the save; a record that fails to SAVE goes to `onEditLogSaveFailure()` – that `catch` used to be silent. Not covered: lite `int[]`/`string[]` fields (no rows of their own) and `dynamic_pics`/`dynamic_files`. Covered by `php/tests/Admin/NestedEditLogTest.php` (the real `submit()` over a throwaway table) and `php/tests/Entity/AdminTableEditLog/NestedRowsLogTest.php`.
...
**TLS is verified** (`MerchantApi::VERIFY_TLS`), and a root missing from the host's store is answered with a bundle — `Helper::getCaBundlePath()` → `CURLOPT_CAINFO` — never by turning verification off: the bundle adds one anchor, the constant removes the protection for every host. `Helper::createApi()` is the seam for substituting an api subclass. Covered by `php/tests/Payment/TinkoffCaBundleTest.php`.

**A transport failure is reported through `getError()`, never thrown** — and that now includes the one branch that used to throw: `curl_init()` returning false. Both clients (Tinkoff, CloudPayments) leave the raw response `false`, clear the parsed fields and return `false`, because every caller already branches on `getError()` — `Helper::getFormUri()` turns it into an `\Exception`, `getPaymentState()`/`getSbpPayload()` into a logged `null`. What it threw before was `HttpException` — a pecl_http v1 class PHP 8 does not have, so the branch raised `Error: Class "HttpException" not found` instead of anything a caller could read. The handle is opened through `MerchantApi::openCurl()`, a second seam next to `Helper::createApi()`, purely so a test can force that branch; a test that exercises it **must assert the override was used**, or it silently POSTs to the live gateway instead of failing. Covered by `php/tests/Payment/TinkoffCurlInitFailureTest.php`.
