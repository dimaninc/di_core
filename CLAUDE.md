...
**Localization migrations add tokens with `LocalizationMigration::insertValues()`** — it takes `'token' => [ru, en, de, it, es, fr]`, a **positional** list matching `getValueFields()` (a short one leaves the rest empty, a long one has its tail ignored), escapes key and values through the DB API and does one `INSERT IGNORE` per token, so a translation an editor already changed survives a re-run. Override `getValueFields()` on a schema with a different language set — this package's own `sql/localization.sql` only has `value` and `en_value`. Covered by `php/tests/Database/LocalizationMigrationInsertValuesTest.php`.
